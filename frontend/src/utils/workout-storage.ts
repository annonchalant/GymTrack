// Local-only persistence for workout sets, keyed by date (YYYY-MM-DD).
// Stored as a single JSON map under one AsyncStorage key for atomic reads/writes.

import { storage } from "@/src/utils/storage";

const KEY = "fit.workouts.v1";

export type WorkoutSet = {
  id: string;
  exercise: string;
  weight: number; // raw number — units (lb/kg) are not tracked in Phase 2.
  reps: number;
...
// Same shape as WorkoutSet but enriched with the storage `dateKey` so consumers
// (e.g. streak calc, calendar marking) don't need to thread it themselves.
