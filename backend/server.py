...
async def get_all_storage():
    items = await db.storage.find().to_list(1000)
    return [{"key": item["key"], "value": item["value"]} for item in items]

@api_router.post("/storage")
async def set_storage_item(item: StorageItem):
...
async def delete_storage_item(key: str):
    await db.storage.delete_one({"key": key})
    return {"status": "success"}
